...
class User{
	...
	protected static $tbl_name = "tbl_candidates";
	function db_fields(){
		global $mydb;
	...
	function find_all_guest($lname="", $email=""){
			global $mydb;
			$mydb->setQuery("SELECT * 
							FROM  ".self::$tbl_name." 
							WHERE  `lname` ='{$lname}' OR email='{email}'");
	...
	function user_login($username="", $pass=""){
			...
			$mydb->setQuery("SELECT * 
							FROM  ".self::$tbl_name." 
							WHERE  `username` ='". $username ."' AND password='". $pass ."' AND account_status='1'");
			...
			$row_count = $mydb->num_rows($cur);//get the number of count
			 ...
			 $found_user = $mydb->loadSingleResult();
	            ...
	            $_SESSION['user'] 		= $found_user->last_name;
